...
public:
    ...
    public:
        ...
        static bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            AddGossipItemFor(player, GOSSIP_ICON_TRAINER, "Remover as transmogrificações", SENDER_REMOVE_MENU, 0);
                ...
                case SENDER_SELECT_VENDOR: // action = slot
                    {
                        ...
                        if (!item)
                        {
                            if (const char* slotname = TransmogDisplayVendorMgr::getSlotName(action, player->GetSession()))
                                session->SendNotification("Nenhum item equipado em %s", slotname);
                            OnGossipHello(player, creature);
                            return true;
                        }
                        const ItemTemplate * itemTemplate = item->GetTemplate();
                        AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, (std::string)"Atualização selecionada; " + TransmogDisplayVendorMgr::getItemName(itemTemplate, session), sender, action);
                        ...
                        {
                            if (const char* slotname = TransmogDisplayVendorMgr::getSlotName(action, player->GetSession()))
                                session->SendNotification("Não há transmogrificações disponíveis para %s", slotname);
                            OnGossipHello(player, creature);
                            return true;
                        }
                        ...
                        AddGossipItemFor(player, GOSSIP_ICON_TALK, "Voltar . . .", SENDER_BACK, 0);
                        SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
                    } break;
                ...
                case SENDER_REMOVE_ALL: // Remove TransmogDisplayVendorMgrs
                    {
                        ...
                        if (removed)
                        {
                            session->SendAreaTriggerMessage("Transmogrificações removidas de itens equipados");
                        }
                        else
                        {
                            session->SendNotification("Você não possui itens transmogrificados equipados");
                        }
                ...
                case SENDER_REMOVE_ONE: // Remove TransmogDisplayVendorMgr from single item
                    {
                        ...
                        if (Item* newItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, action))
                        {
                            if (TransmogDisplayVendorMgr::GetFakeEntry(newItem))
                            {
                                TransmogDisplayVendorMgr::DeleteFakeEntry(player, newItem);
                                if (slotname)
                                    session->SendAreaTriggerMessage("%s a transmogrificação foi removida", slotname);
                            }
                            else if (slotname)
                            {
                                session->SendNotification("Nenhuma transmogrificação em %s", slotname);
                            }
                        }
                        else if (slotname)
                        {
                ...
                case SENDER_REMOVE_MENU:
                    {
                        for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; slot++)
                        {
                            ...
                            if (!slotname)
                                continue;
                            std::ostringstream ss;
                            ss << "Remova a transmogrificação de " << slotname << "?";
                            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, (std::string)"Remover de " + slotname, SENDER_REMOVE_ONE, slot, ss.str().c_str(), 0, false);
                        }
                        AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "Remova todas as transmogrificações", SENDER_REMOVE_ALL, 0, "Tem certeza de que deseja remover todas as transmogrificações?", 0, false);
                        AddGossipItemFor(player, GOSSIP_ICON_TALK, "Voltar . . .", SENDER_BACK, 0);
                        SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
                    } break;
                default: // Show items you can use
                    {
                        ...
                        if (const ItemTemplate* itemTemplate = sObjectMgr->GetItemTemplate(selection.item))
                        {
                            ...
                            {
                                player->GetSession()->SendNotification("O item equipado não é adequado para a transmogrificação");
                            ...
                            if (!itemCount)
                            {
                                session->SendAreaTriggerMessage("Nenhum item encontrado");
                                OnGossipSelect(player, creature, SENDER_SELECT_VENDOR, slot);
                                return true;
                            }
                            ...
                            if (!item_amount)
                            {
                                session->SendAreaTriggerMessage("Não foram encontradas transmogrificações para itens equipados");
                                OnGossipSelect(player, creature, SENDER_SELECT_VENDOR, slot);
                                return true;
                            }
                            else
                        ...
                        {
                            session->SendNotification("Item inválido equipado");
                            OnGossipSelect(player, creature, SENDER_SELECT_VENDOR, slot);
                            return true;
                        }
                    } break;
