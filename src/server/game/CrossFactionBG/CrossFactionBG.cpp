...
void Player::FitPlayerInTeam(bool action, Battleground* pBattleGround)
{
    ...
    if (pBattleGround && action)
        SendChatMessage("%s Você está jogando para a %s%s na batalha de %s", MSG_COLOR_WHITE, GetTeam() == ALLIANCE ? MSG_COLOR_DARKBLUE"aliança" : MSG_COLOR_RED"horda", MSG_COLOR_WHITE, pBattleGround->GetName().c_str());
}
